...
bool CAstcUtils::DecodeToRGBA32(const std::vector<unsigned char>& vASTCData,int vWidth,int vHeight,std::vector<unsigned char>& oRGBAData){
...
bool CAstcUtils::ReadASTC(const std::string &vFilePath, std::vector<unsigned char> &oASTCData, int &oWidth, int &oHeight){
    size_t Size;
...
bool CAstcUtils::parseAstcHeader(const uint8_t *vData, size_t vSize, AstcHeaderInfo &oOut)
{
